...
resize_window_to_half_screen() {
	...
	JQ=$NIXRUNPATH/jq
	SED=$NIXRUNPATH/sed
	eval $(system_profiler SPDisplaysDataType -json|$JQ '.SPDisplaysDataType.[0].spdisplays_ndrvs.[] | select (.spdisplays_main == "spdisplays_yes") | ._spdisplays_resolution' | $SED -rn 's/"([0-9]+)\s*x\s*([0-9]+).*$/XX=\1;YY=\2/p')
